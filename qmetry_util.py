...
class UploadResult:
        ...
        fields = {
            'testCycle': {
                ...
                'summary': ConfigurationsManager().get_str_for_key("automation.qmetry.testcycle.summary") if (
                                                                                                                 ConfigurationsManager().get_str_for_key(
                                                                                                                     "automation.qmetry.testcycle.summary")) != "" else 'Automated Test Cycle',
                'description': ConfigurationsManager().get_str_for_key("automation.qmetry.testcycle.description") if (
                                                                                                                   ConfigurationsManager().get_str_for_key(
                                                                                                                       "automation.qmetry.testcycle.description")) != "" else '',
                'assignee': ConfigurationsManager().get_str_for_key("automation.qmetry.testcycle.assignee") if (
                                                                                                                           ConfigurationsManager().get_str_for_key(
            ...
            'testCase': {
                ...
                'description': ConfigurationsManager().get_str_for_key("automation.qmetry.testcase.description") if (
                                                                                                              ConfigurationsManager().get_str_for_key(
                                                                                                                  "automation.qmetry.testcase.description")) != "" else '',
                'assignee': ConfigurationsManager().get_str_for_key("automation.qmetry.testcase.assignee") if (
                                                                                                                  ConfigurationsManager().get_str_for_key(
    ...
    def getQTM4J4xServerUrl(self, url, Authorization, id, apiKey):
        reqUrl = url + "/rest/qtm4j/automation/latest/importresult/track?trackingId=" + id
        headers = {
    ...
    def uploadFile(self):
        ...
        if (integrationType == 'QTM'):
            ...
            headers['apiKey'] = apiKey
            headers['Authorization'] = "Basic " + str(encodedAuth)
            res1 = requests.request("POST", requestUrl, data=params,
                                    headers=headers, files=files)

            schedule.every(2).seconds.do(
                self.getQTM4J4xServerUrl, url=url.strip('/rest/qtm4j/automation/latest/importresult'), Authorization="Basic " + str(encodedAuth), id=trackingId, apiKey=apiKey)
            while 1:
                schedule.run_pending()
                time.sleep(1)
