				...
				.jqGrid(
						{
								...
								root : function(obj) {
									...
									if ($.isArray(obj.rows)) {
										$.each(obj.rows,function(index, value) {
											var tot = (value["done"]
													+ value["review"]
													+ value["pending"] + value["other"]);
											value["backlog"] = value["noOfScenarios"]
													- tot;

											var scount = value["noOfScenarios"];

											value["coverage"] = Math.ceil(scount > 0 ? (tot * 100 / scount)
															: tot > 0 ? 100: 0);
										});
										obj.rows
												.sort(function(a, b) {
							...
							// width: 780,
							height : '100%',
							rowList : [ "15:15", "20:20", "25:25", "10000:All" ],
							loadonce : true,
							colMenu : true,
							pager : "#jqGridPager",
							multiSort : true,
							gridview : true,
							rowattr : function(rd) {
								var nos = parseInt(rd.noOfScenarios, 10);
								var nod = parseInt(rd.done, 10)
										+ parseInt(rd.review, 10)
										+ parseInt(rd.pending, 10)
										+ parseInt(rd.other, 10);
								var nop = nos - nod;
								if (nos === 0 && nos === nod) {
									return {
										"class" : "ui-state-warn"
							...
							gridComplete : function() {
								var $grid = $('#jqGrid');
								var storyCount = $grid.jqGrid('getCol',
										'story', false, 'count');
								var scenarioCount = countif($grid.jqGrid(
										'getCol', 'noOfScenarios', true),
										function(v) {
											return v > 0;
										});
								var automatedCount = countif($grid.jqGrid(
										'getCol', 'coverage', true),
										function(v) {
											return v != "0%";
										});
								var noOfScenariosSum = $grid.jqGrid('getCol',
										'noOfScenarios', false, 'sum');
								var doneSum = $grid.jqGrid('getCol', 'done',
										false, 'sum');
								var reviewSum = $grid.jqGrid('getCol',
										'review', false, 'sum');
								var pendingSum = $grid.jqGrid('getCol',
										'pending', false, 'sum');
								var otherSum = $grid.jqGrid('getCol', 'other',
										false, 'sum');
								var backlogSum = noOfScenariosSum
										- (doneSum + reviewSum + pendingSum + otherSum);
								// var temp = scenarioCount + "/" +storyCount
								// +"-"+scenarioCount*100/storyCount + "%";
								$grid.jqGrid('footerData', 'set', {
									qaStatus : scenarioCount
											+ "/"
											+ storyCount
											+ "-"
											+ Math.ceil(scenarioCount * 100
													/ storyCount) + "%",
									coverage : 0,
									story : automatedCount
											+ "/"
											+ storyCount
											+ "- "
											+ Math.ceil(automatedCount * 100
													/ storyCount) + "%",
									noOfScenarios : noOfScenariosSum,
									done : doneSum,
									review : reviewSum,
									pending : pendingSum,
									other : otherSum,
								...
								var ts = $grid.jqGrid('getGridParam', 'ts');
								userdata = $grid.jqGrid('getGridParam',
										'userData');
								var date = new Date(userdata.ts);

								$grid.jqGrid('setCaption', 'Coverage as of - '
										+ date.toString());
							}
						});
...
function countif(items, cond) {
	var count = 0;
	for (var i = 0; i < items.length; ++i) {
		if (cond(items[i].value))
			count++;
	}
	return count;
}

function generateExecutionCoverage(dir) {
	if ($('#executionCoverageGrid').is(":empty")) {

		$("#executionCoverageGrid")
				.jqGrid(
						{
							url : dir + '/scanarioCoverage.json',
							mtype : "GET",
							datatype : "json",
							// root : "entires",
							colModel : [
							...
							// rowNum:1000,
							colMenu : true,
							pager : "#executionCoverageGridPager",
									...
									if ($.isArray(obj.rows)) {
										$.each(obj.rows,function(index, value) {
											var tot = (value["pass"]
													+ value["fail"]
													+ value["skip"]);
											value["passRate"] = Math.ceil(value["pass"] > 0 ? (value["pass"] * 100 / tot)
													: 0);
										...
										obj.rows.sort(function(a, b) {
											var atot = (a["pass"]+ a["fail"]+ a["skip"]);
											return b["passRate"] - a["passRate"] || b["coverage"] - a["coverage"]
													|| b["noOfScenarios"]
													- a["noOfScenarios"];
										});
										return obj.rows;
							...
							loadComplete : function() {
								...
								var passSum = $grid.jqGrid('getCol', 'pass',
										false, 'sum');
								var failSum = $grid.jqGrid('getCol', 'fail',
										false, 'sum');
								var skipSum = $grid.jqGrid('getCol', 'skip',
										false, 'sum');
								var coverageSum = Math
										.ceil((passSum + failSum + skipSum)
												* 100 / noOfScenariosSum);
		...
		// activate the toolbar searching
		$('#executionCoverageGrid').jqGrid('navGrid',
				"#executionCoverageGridPager", {
					search : true, // show search button on the toolbar
					add : false,
					edit : false,
					del : false,
		...
		$("#search_cells").on("keyup", function() {
			...
			timer = setTimeout(function() {
				// timer = null;
				$("#executionCoverageGrid").jqGrid('filterInput', self.value);
			}, 0);
		});
